function releaseCallback(e,t,n){if(n.status!==200||n.responseText.length<10)throw document.getElementById(`last-updated-${e}`).innerText="never",document.getElementById(`unavailable-${e}`).classList.remove("is-hidden"),document.getElementById(`downloads-${e}`).classList.add("is-hidden"),new Error(n.responseText);var s,o,i,a,r,l,u,h,c=JSON.parse(n.responseText),d=0;document.getElementById(`last-updated-${e}`).innerText=moment(c[0].published_at).fromNow();for(o=0;o<c.length;++o){if(s=c[o],l=!1,s.assets.forEach(function(e){if(e.name.includes("-mingw-"))return;e.name.includes("windows")&&(l=!0)}),!l)continue;if(u=moment(s.published_at).fromNow(),i=s.target_commitish,h=`https://github.com/yuzu-emu/yuzu-${e}/commit/${i}`,a="",e=="mainline"&&(a="Mainline Build"),i&&(a+=" - "+i.substring(0,9)),r="",s.assets.forEach(function(t){if(t.name.includes("nupkg"))return;if(t.name.includes(".7z"))return;if(t.name.includes("RELEASES"))return;if(t.name.includes(".tar.xz"))return;if(t.name.includes("-debugsymbols.zip"))return;if(t.name.includes(".zsync"))return;if(t.name.includes("-mingw-"))return;if(t.name.includes("osx"))return;var o,n="unknown";t.name.includes("windows")?n="windows":t.name.includes("exe")?n="windows":t.name.includes("osx")?n="apple":t.name.includes("AppImage")&&(n="linux"),o=`https://github.com/yuzu-emu/yuzu-${e}/releases/download/${s.tag_name}/${t.name}`,r+=`
            <a class="level-item">
                <a href="${o}">
